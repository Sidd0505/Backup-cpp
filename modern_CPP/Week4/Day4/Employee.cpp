...
void FindSalaryById(const Container& data, int id)
{
    if(data.empty()){
        std::cout<<"data is empty\n";
        }
    //Option - 1 //which is wrong  don't need find if on map
...
void FindAverageSalaryForGivenEmployees(const Container& data, std::vector<int>& ids)
{
    //option 1 : this solution is not feasible complexity : o(mn)
    for(int id :ids)
    {
        float total = 0.0f;
        int count =0;
        ...
        if(itr != data.end()){
            count++;
            total = total + itr->second->salary();
            std::cout<<"Total salary for given ids :"<<total<<"\n";   
        }
    }
...
/*
   Time complexity for searching in map : Amortized (Adjusted) o(1);

*/
void FindAverageSalaryForGivenEmployeesImproved(const Container& data, std::vector<int>& ids)
{
    //option 2
    for(int id :ids)
    {
...
/*
    Tress we need to implement -
    AVL, red black, binary, binary search 


*/
/*
string operations like sorting
