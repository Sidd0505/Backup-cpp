...
void Flight::feedInfo(){                            //take input Flight Information from user  
...
void Flight::showInfo(){                            //Display Flight Information
    std::cout<<"Flight No.: "<<flight_no<<std::endl;
    std::cout<<"Flight Distance: "<<distance<<std::endl;
    std::cout<<"Flight Fuel : "<<fuel<<std::endl;
    std::cout<<"Flight Fare : "<<fare<<std::endl<<std::endl;
}

float Flight::calculateFuelQuantity(){              //Calculate Fuel Quantity
    float fuel_quan;
    if(distance<=10000){
        fuel_quan=5000;
...
void Flight::calculateTotalFlightObjects(){  //Display total flights
    if(objs!=0){
        std::cout<<"Total Flights: "<<objs<<std::endl;
    }else std::cout<<"No Flights Here";
}

void search(Flight a[],int fli_no,int num){         //Search Function
    int flag=0;
    for(int i=0;i<num;i++){
        if(a[i].flightNo()==fli_no){
            a[i].showInfo();
            flag=1;
        }
    }
    if(flag==0) std::cout<<"Flight Not Found"<<std::endl;
}//main.cpp
